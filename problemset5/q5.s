...
main:   
	...
	# 128: 74.43233
	# 200: 1825.4126
	li      a2, 64		# change the immediate for a different array size 
	la	a0, fvalues
	addi	a1, zero, 32
	add	a1, a0, a1
	jal	dot_product
...
# 	return sum;
# }
# Translate the above C code into RISC-V assembly code
dot_product:
	# TODO
    # use one of the fcvt instructions to convert
    # integer 0 to SP 0
	addi   sp, sp, -16 # allocate space for 2 floats
	sw     ra, 12(sp) # save ra
	sw     s0, 8(sp) # save s0
	sw     s1, 4(sp) # save s1
	sw     s2, 0(sp) # save s2
...
loop:
	...
	addi	 sp, sp, 16 # deallocate space for 2 floats
	jr    ra # return
