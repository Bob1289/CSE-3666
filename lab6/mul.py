...
@block 
def RegisterShiftLeft(dout, en, load_data, load, clock, reset):
...
@block 
def RegisterShiftRight(dout, en, load_data, load, clock, reset):
...
def Mul_ww(p, x_init, y_init, load, done, clock, reset):
    ...
    p_en = Signal(bool(0))  
    shift_en = Signal(bool(0)) # x and y have the same register
    ...
    reg_x = RegisterShiftLeft(x, shift_en, x_init, load, clock, reset)
    reg_y = RegisterShiftRight(y, shift_en, y_init, load, clock, reset)
    adder = Adder(adder_out, x, p)
    contorl = Control(done, p_en, shift_en, test, load, clock, reset)
    # x_init and y_init are the load_data signal to reg_x and reg_y, respectively

    # set the test signal going into the control module
    @always_comb
    def testgen():
        #TODO:
        # set test signal
