...
def ALU1bit(a, b, carryin, binvert, operation, result, carryout):
    ...
    @always_comb
    def comb_and():
        and_out.next = mux1_out and a

    # the OR gate
    @always_comb
    def comb_or():
        or_out.next = mux1_out or a
    # adder.
    ...
    @always_comb
    def comb_adder():
        adder_sum.next = ((not a) and (not mux1_out) and carryin) or ((not a) and (mux1_out) and (not carryin)) or (a and (not mux1_out) and (not carryin)) or (a and mux1_out and carryin)
        carryout.next = (carryin and mux1_out) or (a and carryin) or (a and mux1_out)
